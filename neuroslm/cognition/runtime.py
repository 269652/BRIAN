...
def format_debug_trace(result: TickResult) -> str:
    ...
    actions generated by basal ganglia": every candidate thought this
    tick generated, its trunk NLL, and which one won; the hippocampal
    episodes that shaped THINK; and the thought's lineage from the
    prior tick, so a running log shows how thoughts evolve over time.
    Meant for a raw debug stream (``log_stream``), not the compact
    wire-protocol summary (:func:`format_introspection`).
    """
    mode = "DMN wandering" if result.wandering else "responding"
    lines = [f"[tick {result.tick_n}] {mode}"]
    ...
    lines.append("  BG deliberation:")
    for cand, sc in zip(result.candidates, result.scores):
        mark = "SELECTED" if cand == result.thought else "        "
        lines.append(f"    [{mark}] nll={sc.mean_nll:5.2f} "
                     f"\"{_truncate(cand)}\"")
