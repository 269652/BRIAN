...
### 9.7 Liouville Symplectic Residual (H25, Phase 2 of THSD program)

**Mechanism:** The final hidden state `h ∈ ℝ^{B×T×d_model}` is split into canonical coordinates `(q, p)` of equal dimension `d_model/2`. One explicit Stoermer-Verlet (leapfrog) step is applied under a learned Hamiltonian:

```
H(q, p) = (1/2) ||M^{-1/2} p||² + V(q) + W(q)
```

where `M = softplus(raw_M)` is a learnable diagonal mass matrix, `V(q)` is a q-only potential (`QuadraticPotential` in diagnostic mode, `SwiGLUPotential` in production), and `W(q)` is a `LowRankPairwise` token-interaction term. The three leapfrog substeps are triangular shears with `det = 1` each, so their composition satisfies **Liouville's theorem** (`det(J) = 1` exactly). The `QOnlyPotential` abstract base enforces this at the type level: any subclass whose `forward` signature accepts `p` raises a `TypeError` at construction.

The Noether residual `L_Noether = (H_final − H_initial)²` is the diagnostic/loss term. In the `analytic` (quadratic V, M = I) limit this equals zero at machine precision; for a general potential it is finite-and-bounded by the Hairer-Lubich-Wanner modified-Hamiltonian theorem (Geometric Numerical Integration, 2006, Ch. IX, Thm 8.1).

DSL surface: `regularization { liouville_symplectic: { enabled: true, noether_strength: λ, dtau_init: τ, potential_kind: "quadratic"|"swiglu", w_rank: r } }`. With `λ = 0` (default) the mechanism is **diagnostic-only**: `noether_loss` and `noether_H_diff` are logged every step, zero added to the loss budget.

**Implementation:** `neuroslm/mechanisms/liouville_symplectic.py` (library — `QOnlyPotential`, `QuadraticPotential`, `QuadraticHarmonicPotential`, `SwiGLUPotential`, `LowRankPairwise`, `LiouvilleSymplecticBlock`), `neuroslm/dsl/regularization.py` (`LiouvilleSymplecticConfig`, 9th `RegularizationConfig` entry), `neuroslm/regularizers.py` (`RegularizationController.collect_symplectic_aux()` — lazy-builds block from `h_last.shape[-1]`), `neuroslm/harness.py` (`BRIANHarness._symplectic_aux_step()` — auto-fired after `_topo_charge_aux_step`).

**TDD evidence:** 39 GREEN tests across 4 files including the fp64 `det(J) = 1` invariant (atol=1e-9), Hairer-Lubich-Wanner H-oscillation bound over 100 steps of a 1-D harmonic oscillator, FD-vs-autograd grad on `dtau`, `QOnlyPotential` type-rejection, and `torch.equal` inert-gate (noether_strength=0). See `docs/findings.md` H25 for the falsifiable predictions.
