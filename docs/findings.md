...
**Same-day addendum 4 — DMN mind-wandering + basal-ganglia debug
trace (2026-08-12).** "Make the DMN loop cause introspective mind
wandering thoughts... debug log how thoughts evolve over time...
debug log the actions generated by basal ganglia." Two additions to
`neuroslm/cognition/runtime.py`:

- **Mind-wandering.** `TickResult.wandering` is `True` exactly when a
  tick had no sensory input that cycle — the DMN condition. Those
  ticks bias `_compose_prompt` toward one of six rotating
  `MindConfig.wander_prompts` (reflective/associative framing —
  "let my mind wander...", "following a tangent...") instead of
  completing a flat prompt; ticks responding to real input never see
  this framing (task-positive vs DMN is a real behavioral branch, not
  cosmetic).
- **Debug trace.** `TickResult` gained `tick_n` (monotonic cycle
  counter) and `prior_thought` (lineage). `format_debug_trace(result)`
  renders the full basal-ganglia deliberation the compact
  `format_introspection` summary doesn't show: every candidate + its
  trunk NLL, `[SELECTED]` marking the winner, the hippocampal episodes
  that shaped THINK, and an `evolved from:` line. Written to
  `log_stream` (alongside the existing compact summary) on every tick
  — `brian logs <box>` now shows the full deliberation, not just the
  outcome.

[EVIDENCE: tests/cognition/test_cognitive_runtime.py (51) green — 11 RED-confirmed before implementation]

### The A100 mind box was running its expert on CPU the whole time (2026-08-12)

Live report: `/think` returned `None`, and the DMN loop only produced
one thought roughly every ~58s instead of the ~12s `thought_period` —
on a box explicitly rented for A100.

**Root cause.** `AutoModelForCausalLM.from_pretrained()` loads on CPU
by default; `build_runtime_from_hf_lm` never called `.to(device)`, so
every `--expert` deploy silently ran the whole model on CPU regardless
of the GPU it was billed for. Compounding: `vast_mind.py`'s onstart
never passed `--device` at all, and `brian chat`'s own CLI default for
`--device` is `cpu` — even a correct fix to `build_runtime_from_hf_lm`
alone would have kept defaulting to CPU on every mind deploy.

**Why `/think` returned `None`, specifically.** `ChatDaemon.think_once`
uses a non-blocking lock (`acquire(blocking=False)`) so a busy tick
never stalls a caller — by design. But at ~55-60s/tick (CPU-bound
1.5B inference: 3 candidates generated + 3 candidates re-scored), the
background thought thread holds that lock for nearly the entire
12s-nominal period between ticks. Any client `/think` landing in that
window bounced off the lock and got `None` — not a bug in the gating
logic, a symptom of the tick simply taking too long.

**Fix (two parts, both required — verified live against the RED
symptom sequence).**
1. `neuroslm/cognition/runtime.py::build_runtime_from_hf_lm`: `model =
   model.to(device)` unconditionally after `.eval()` (not an
   `if device != "cpu"` branch — that shape is exactly what let this
   drift in unnoticed).
2. `neuroslm/connectors/vast_mind.py`: `MindDeployConfig.device`
   (default `"cuda"`) threaded into the onstart's `chat` invocation
   (`--device 'cuda'`); `brian deploy-mind --device ...` exposes the
   override.

**Live confirmation, same box family:** the PRE-fix box (`47505933`)
logged ticks 58s apart (`00:21:32` → `00:22:30` → `00:23:28`) and
`/think` returning `None`. Redeploy with this fix required to confirm
tick cadence drops to seconds — tracked as the immediate follow-up,
