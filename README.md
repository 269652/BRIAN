...
[![tests](https://img.shields.io/badge/tests-3037%20passing-brightgreen)](#tests)
...
**Current status:**
- ✅ **Layer A (mechanisms):** 20+ core properties verified via 3037 unit tests across `tests/` (`tests/dsl/` alone runs 860). All mechanisms compute as specified, including the new **cortex_pre_head_norm catastrophic-loss fix**, **KL-distillation aux loss**, **NT-mediated α gating**, **ImprovementGate** (Welch's t-test admission), and **TheoryOfMindIR**.
...
**Why?** Hand-written PyTorch is error-prone for biologically-plausible models; math specs are checkable. The DSL codegen produces torch modules with **byte-equivalent forward passes** (verified by 860 DSL tests in `tests/dsl/`) and enables symbolic analysis (fixed-point, stability, sensitivity). The DNA layer (`neuroslm/compiler/module_bundler.py`, `ribosome.py`) supports **module bundling with source maps** and **byte-identity round-trip** verification (`tests/test_dna_roundtrip_byte_identity.py`).
...
**Run all:** `py -3 -m pytest tests/ -v` (3037 tests, ~379 seconds on CPU)
...
- **3037 tests passing** in `tests/` (~379s on CPU); breakdown: **860 in `tests/dsl/`** (DSL parsing + codegen + byte-equivalence), **294 in `tests/training/`** (harness, multi-cortex, distillation, gating), plus verification, THSD, evolution, narrative, qualia, neurochem subsuites.
