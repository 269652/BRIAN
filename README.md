...
[![tests](https://img.shields.io/badge/tests-3150%20passing-brightgreen)](#tests)
...
**Current status:**
- ✅ **Layer A (mechanisms):** 20+ core properties verified via 3150 unit tests across `tests/` (`tests/dsl/` alone runs 936). All mechanisms compute as specified, including the new **cortex_pre_head_norm catastrophic-loss fix**, **KL-distillation aux loss**, **NT-mediated α gating**, **ImprovementGate** (Welch's t-test admission), and **TheoryOfMindIR**.
...
**Run all:** `py -3 -m pytest tests/ -v` (3150 tests, ~393 seconds on CPU)
...
- **3150 tests passing** in `tests/` (~393s on CPU); breakdown: **936 in `tests/dsl/`** (DSL parsing + codegen + byte-equivalence), **294 in `tests/training/`** (harness, multi-cortex, distillation, gating), plus verification, THSD, evolution, narrative, qualia, neurochem subsuites.
