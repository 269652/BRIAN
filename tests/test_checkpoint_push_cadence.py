...
# ─────────────────────────────────────────────────────────────────────
# 4. brian deploy — propagate cadence knobs as env vars
# ─────────────────────────────────────────────────────────────────────


@pytest.fixture
def _isolated_cfg(tmp_path: Path, monkeypatch):
    """Pin ``load_project_config`` to a tmp ``brian.toml`` and clear
    every BRIAN_* / cadence env so the test starts from a known state."""
    for k in (
        "BRIAN_ARCH", "BRIAN_DNA",
        "BRIAN_DEFAULT_STEPS", "BRIAN_DEFAULT_PRESET",
        "BRIAN_DEFAULT_HARDWARE", "BRIAN_DEFAULT_BRANCH",
        "BRIAN_DEFAULT_LOG_EVERY", "BRIAN_DEFAULT_SAVE_EVERY",
        "BRIAN_DEFAULT_PUSH_EVERY",
        "BRANCH", "STEPS", "ARCH", "BRIAN_SOURCE_DNA",
        "LOG_EVERY", "SAVE_EVERY", "PUSH_EVERY",
    ):
...
def _capture_subprocess(monkeypatch):
    """Snoop on subprocess.call so ``cmd_deploy`` doesn't actually
    launch ``_deploy_train.py``."""
    calls: List[dict] = []
...
def _build_deploy_args(**overrides) -> argparse.Namespace:
...
class TestDeployPropagatesCadenceEnv:
    """``brian deploy`` must export ``LOG_EVERY`` / ``SAVE_EVERY`` /
    ``PUSH_EVERY`` so the on-box trainer can honour them."""

    def test_env_propagated_from_brian_toml(
            self, _isolated_cfg, _capture_subprocess):
        _isolated_cfg(
            '[current]\n'
            'arch = "architectures/master"\n'
            'dna  = ""\n'
        ...
        rc = cmd_deploy(_build_deploy_args())
        assert rc == 0
        env = _capture_subprocess[0]["env"]
        assert env.get("LOG_EVERY") == "50", (
            f"expected LOG_EVERY=50 from brian.toml, got "
            f"{env.get('LOG_EVERY')!r}"
        )
        assert env.get("SAVE_EVERY") == "250", (
            f"expected SAVE_EVERY=250 from brian.toml, got "
            f"{env.get('SAVE_EVERY')!r}"
        )
        assert env.get("PUSH_EVERY") == "500", (
            f"expected PUSH_EVERY=500 from brian.toml, got "
            f"{env.get('PUSH_EVERY')!r}"
        )

    def test_env_defaults_when_brian_toml_silent(
            self, _isolated_cfg, _capture_subprocess):
        """Even with empty [defaults], deploy must set sane env
        values so the on-box trainer never silently uses a stale
        SAVE_EVERY=1000."""
        _isolated_cfg(
            '[current]\n'
            'arch = "architectures/master"\n'
        ...
        assert rc == 0
        env = _capture_subprocess[0]["env"]
        # Sensible no-config defaults; must match the
        # ProjectConfig defaults above.
        assert env.get("LOG_EVERY")  == "20"
        assert env.get("SAVE_EVERY") == "500"
        assert env.get("PUSH_EVERY") == "500"
