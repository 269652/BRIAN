...
5. [Wiring Diagram — NeuralOrchestrator Re-entrant Loops](#5-wiring-diagram--neuralorchestratorrehensive-re-entrant-loops)
...
| Loop | Trigger | Stages revisited |
|---|---|---|
| Effort loop | `ACC.effort_steps > 0` | 6 → 7 → 8 (up to `max_thinking_steps=12`) |
| Vesicle tick | Every forward pass | Modulates all stages via dock signal |
| Floating thought EMA | Every tick | Feeds back into Stage 1 as prior context |
| Trophic update | Every train step | Adjusts projection gains across all stages |
...
Seven NTs with Euler-integrated dynamics (per tick):
...
| GABA | 0.50 | 0.40 | Inhibition |

$$\text{level}_i(t+1) = \tau_i \cdot \text{level}_i(t) + \text{release}_i(t)$$

The `TransmitterSystem` returns `float32` tensors. **All modules that receive NT tensors must cast them to the model dtype** before arithmetic — this is enforced at module boundaries (Thalamus, GWS, ReceptorBank.modulate).
...
**Adafactor optimizer** (TPU-native, no momentum state): reduces optimizer memory by 4× vs Adam, critical for fitting xl-sized models on a single TPU core.
...
*Last updated: 2026-05-10. Source of truth: `neuroslm/` on branch `tpu`.*
